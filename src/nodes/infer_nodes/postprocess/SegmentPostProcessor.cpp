    ...
    int SegmentPostProcessor::run(std::vector<FBlob> &outputs, std::vector<std::shared_ptr<FrameROI>> &frame_rois)
    {
        ...
        for(int i = 0; i < outputs.size(); i++)
        {
            ...
            for(int j = 0; j < bs; j++)
            {
                ...
                std::shared_ptr<FrameData> frame_mask = std::make_shared<FrameData>(frame_roi->frame->width, frame_roi->frame->height, 1, ZJV_PIXEL_DTYPE_UINT8);
                memcpy(frame_mask->data->mutable_cpu_data(), original_mask.data(), original_mask.size() * sizeof(unsigned char));

                std::shared_ptr<FrameData> frame_conf = std::make_shared<FrameData>(frame_roi->frame->width, frame_roi->frame->height, 1, ZJV_PIXEL_DTYPE_FLOAT32);
